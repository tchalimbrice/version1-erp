...
@Component({
  ...
  template: `
<div style="min-height:100vh;background:linear-gradient(135deg,#1a5c2a 0%,#0d3318 100%);display:flex;align-items:center;justify-content:center;padding:20px;position:relative;overflow:hidden;">
  <div style="position:absolute;inset:0;background:url('https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=1600&auto=format&fit=crop') center/cover no-repeat;opacity:.16;pointer-events:none;"></div>
  <div style="background:#fff;border-radius:22px;width:420px;max-width:100%;box-shadow:0 24px 64px rgba(0,0,0,.4);overflow:hidden;position:relative;z-index:1;">
    ...
    <div style="background:linear-gradient(135deg,#1a5c2a,#0d3318);padding:34px;text-align:center;">
      <div style="width:68px;height:68px;background:linear-gradient(135deg,#e67e22,#d35400);border-radius:18px;display:grid;place-items:center;font-size:32px;margin:0 auto 16px;box-shadow:0 4px 16px rgba(0,0,0,.25);">🌾</div>
      ...
      <div style="font-size:22px;font-weight:900;color:#fff;letter-spacing:.5px;">Gestion de Ferme</div>
      @if (companyName()) {
    ...
    <div style="padding:30px 34px;">
      ...
      @if (hasInvite()) {
        <div style="display:flex;flex-direction:column;gap:15px;">
          ...
          <button (click)="login()"
            style="background:linear-gradient(135deg,#1a5c2a,#27ae60);color:#fff;border:none;border-radius:11px;padding:13px;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 4px 12px rgba(26,92,42,.35);">
            Se connecter →
          </button>
        </div>
      } @else {
...
export class LoginComponent {
  ...
  private redirectByRole(role: string) {
    const dest: Record<string, string> = {
      owner: '/tableau-de-bord', employee: '/animaux', hr: '/rh', accountant: '/comptabilite', pharmacien: '/pharmacie'
    };
    this.router.navigate([dest[role] ?? '/tableau-de-bord']);
  }
}
