...
export type InviteDomain = 'gestion-hotellerie';
...
export function decodeInvite(raw: string): InvitePayloadV1 | undefined {
  try {
    ...
    if (!parsed || parsed.v !== 1 || parsed.domain !== 'gestion-hotellerie') return undefined;
    if (!parsed.company || !parsed.user || !parsed.mode || !parsed.password) return undefined;
    return parsed;
  } catch {
    return undefined;
  }
