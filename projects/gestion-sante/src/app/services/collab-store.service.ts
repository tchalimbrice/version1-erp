...
export interface Collaborator {
  ...
  email: string; role: Role; password: string; token: string;
}
...
const STORAGE_KEY = 'bizmaster_sante_state';
...
export class CollabStoreService {
  ...
  private save() {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({
      ...
      currentUser: this.currentUser(), collaborators: this.collaborators(), activities: this.activities()
    }));
  }
