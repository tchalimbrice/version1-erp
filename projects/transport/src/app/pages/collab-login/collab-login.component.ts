...
export class CollabLoginComponent {
  ...
  connect() {
    ...
    if (role === 'employee') this.router.navigate(['/trajets']);
    if (role === 'hr') this.router.navigate(['/chauffeurs']);
    if (role === 'accountant') this.router.navigate(['/tickets']);
    if (role === 'owner') this.router.navigate(['/dashboard']);
  }
}
